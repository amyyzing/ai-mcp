...
const LUACID_MAX_RESPONSE_BYTES = 8 * 1024 * 1024;
...
function luacidOptions(provider: DecompilerProviderSettings): Record<string, string | number | boolean> {
  const excluded = new Set(["transport", "transportExplicit"]);
  return Object.fromEntries(
    Object.entries(provider.options).filter(([key, value]) =>
      !excluded.has(key) && ["string", "number", "boolean"].includes(typeof value)
    )
...
async function runLuacid(
  ...
  const useWebSocket = provider.options.transport === "websocket" || (!explicitHttp && Boolean(provider.apiKey));
  if (!useWebSocket) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      socket = new WebSocket(url, {
        ...
        maxPayload: LUACID_MAX_RESPONSE_BYTES,
      });
    } catch (error) {
      resolve({ ok: false, error: error instanceof Error ? error.message : String(error), latencyMs: Date.now() - startedAt });
      return;
    }
    let settled = false;
    const finish = (result: ProviderRunResult, terminate = false) => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
    ...
    socket.once("open", () => {
      const payload = JSON.stringify({ id: "decompile", encoded_bytecode: bytecodeBase64, ...options });
      socket.send(payload, (error) => {
        if (error) finish({ ok: false, error: error.message, latencyMs: Date.now() - startedAt }, true);
      });
    });
    socket.once("message", (data) => {
      if (rawDataBytes(data) > LUACID_MAX_RESPONSE_BYTES) {
        finish({ ok: false, error: "Luacid WebSocket response exceeded 8 MiB.", latencyMs: Date.now() - startedAt }, true);
        return;
      }
      try {
        const response = JSON.parse(rawDataText(data)) as { decompilation?: unknown; error?: { message?: unknown } };
        ...
        finish({ ok: false, error: "Invalid Luacid WebSocket response.", latencyMs: Date.now() - startedAt }, true);
      }
    });
    socket.once("error", (error) => finish({ ok: false, error: error.message, latencyMs: Date.now() - startedAt }, true));
    socket.once("close", () => {
      if (!settled) finish({ ok: false, error: "Luacid WebSocket closed before returning a result.", latencyMs: Date.now() - startedAt });
    });
  });
}

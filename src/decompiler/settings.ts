...
export const DECOMPILER_PROVIDER_INFO: DecompilerProviderInfo[] = [
  ...
  },
  {
    id: "luaexpert",
    label: "lua.expert",
    description: "Remote lua.expert decompiler. Disabled until the user explicitly opts in to sending bytecode off-device.",
    local: false,
    requiresApiKey: false,
  ...
  },
  {
    id: "shiny",
    label: "Shiny",
    description: "Shiny decompiler using either a local server or the hosted Medal Server endpoint. Hosted mode requires explicit opt-in.",
    local: false,
    requiresApiKey: false,
  ...
  },
  {
    id: "luacid",
    label: "Luacid",
    description: "Remote Luacid HTTP or WebSocket decompiler. Enabling it sends bytecode off-device; WebSocket access requires a paid API key.",
    local: false,
    requiresApiKey: false,
    bodyFormat: "luacid",
  },
  ...
  },
  {
    id: "konstant",
    label: "Konstant",
    description: "Legacy remote Konstant endpoint. Disabled by default because it uses plaintext HTTP.",
    local: false,
    requiresApiKey: false,
...
function normalizeLuacidOptions(value: unknown, fallback: Record<string, unknown>): Record<string, unknown> {
  const input = isObject(value) ? value : {};
  const normalized: Record<string, unknown> = { ...LUACID_DEFAULT_OPTIONS, ...fallback };
  for (const key of Object.keys(LUACID_DEFAULT_OPTIONS)) {
    const candidate = input[key];
    if (key === "transport") {
      if (candidate === "auto" || candidate === "http" || candidate === "websocket") normalized[key] = candidate;
    } else if (key === "transportExplicit") {
      if (typeof candidate === "boolean") normalized[key] = candidate;
    } else if (typeof LUACID_DEFAULT_OPTIONS[key] === "boolean") {
      if (typeof candidate === "boolean") normalized[key] = candidate;
    } else if (typeof candidate === "string" && candidate.length <= 80) {
      normalized[key] = candidate;
    } else if (typeof candidate === "number" && Number.isFinite(candidate) && Math.abs(candidate) <= 1_000_000) {
      normalized[key] = candidate;
    }
  }
  return normalized;
...
function normalizeProvider(
  ...
  return {
    ...
    options: id === "luacid"
      ? normalizeLuacidOptions(input.options, fallback.options)
      : normalizeOptions(input.options, fallback.options),
  };
}

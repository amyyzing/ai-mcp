...
function buildDashboardMcpRelaySnippet(bridgeUrl) {
    ...
    const relayUrl = /^https?:\/\//i.test(normalized) ? normalized : 'http://' + normalized;
    return '{\n' +
        '  "mcpServers": {\n' +
        '    "roblox-mcp": {\n' +
        '      "args": [\n' +
...
function renderHarnessManualInstall() {
    ...
    addClientBody.innerHTML = '<div class="add-client-panel">' +
        ...
        '<div class="add-client-result"><div class="add-client-code-wrap">' +
        '<div class="add-client-code-head"><span class="add-client-code-label">mcpServers.json</span>' +
        '<button class="add-client-btn add-client-btn--ghost" data-action="copy-harness-recipe"' + (recipe ? '' : ' disabled') + '>Copy</button></div>' +
        '<pre class="add-client-code" id="addClientHarnessRecipe" data-copy-text="' + escapeHtml(recipe) + '">' +
        escapeHtml(recipe || 'MCP recipe is unavailable.') + '</pre></div></div></div>';
}

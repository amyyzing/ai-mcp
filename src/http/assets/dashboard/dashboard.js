...
const decompilerProviderUi = {
    ...
    luacid: {
        ...
        description: 'Remote decompiler with keyless HTTP and paid WebSocket access. Enabling it sends bytecode to Luacid.',
...
/* ── Helpers ──────────────────────────────────────────────── */
function getInitials(name) { return String(name || '').slice(0, 2).toUpperCase(); }
...
function avatarHtml(userId, name, size) {
    ...
    if (Number.isSafeInteger(numericUserId) && numericUserId > 0) {
        return `<img src="/api/avatar?userId=${encodeURIComponent(numericUserId)}" alt="${escapeHtml(getInitials(name))}" style="width:${sz}px;height:${sz}px;object-fit:cover;">`;
    }
...
function renderDropdownClients(filter) {
    ...
    clientDropdownList.innerHTML = filtered.map(c => {
        ...
        return `<div class="client-dropdown-item${active}" data-cid="${escapeHtml(c.clientId)}">
            <div class="client-dropdown-item-avatar">${avatarHtml(c.userId, c.username)}</div>
            <div class="client-dropdown-item-info">
                <div class="client-dropdown-item-name">${escapeHtml(c.username)}</div>
                <div class="client-dropdown-item-place">${escapeHtml(c.placeName)}</div>
            </div>
            <span class="client-dropdown-item-transport ${transportClass(c.transport)}">${escapeHtml(c.transport)}</span>
        </div>`;
    }).join('');
...
function renderNoClientList(filter) {
    ...
    noClientList.innerHTML = filtered.map(c => {
        return `<div class="no-client-item" data-cid="${escapeHtml(c.clientId)}">
            <div class="no-client-item-avatar">${avatarHtml(c.userId, c.username, 32)}</div>
            <span class="no-client-item-name">${escapeHtml(c.username)}</span>
            <span class="no-client-item-transport ${transportClass(c.transport)}">${escapeHtml(c.transport)}</span>
        </div>`;
    }).join('');
...
function renderOverviewClients() {
    ...
    el.innerHTML = clients.map(c => {
        return `<div class="section-client" data-cid="${escapeHtml(c.clientId)}">
            <div class="section-client-avatar">${avatarHtml(c.userId, c.username, 32)}</div>
            <div class="section-client-info">
                <div class="section-client-name">${escapeHtml(c.username)}</div>
                <div class="section-client-meta">${escapeHtml(c.placeName)} · ${escapeHtml(c.clientId.slice(0, 8))}…</div>
            </div>
            <span class="section-client-transport ${transportClass(c.transport)}">${escapeHtml(c.transport)}</span>
        </div>`;
    }).join('');
...
function luacidProviderModalHtml(provider) {
    ...
    return `
        ...
        <div class="decompiler-advanced-grid" id="decompilerAdvancedFields" ${decompilerAdvancedOpen ? '' : 'hidden'}>
            <div class="settings-field"><label>Transport</label><select id="decompilerModalLuacidTransport">
            ...
            <div class="settings-field"><label>HTTP endpoint</label><input type="url" id="decompilerModalLuacidEndpoint" value="${escapeHtml(provider.endpoint || 'https://api.luacid.dev/decompile')}" placeholder="https://api.luacid.dev/decompile"></div>
            <div class="settings-field"><label>Decompiler options JSON</label><textarea id="decompilerModalLuacidOptions" rows="7" placeholder='{"indent":"tab"}'>${escapeHtml(formatSettingsJson(options))}</textarea></div>
        </div>
        <div class="decompiler-modal-note">Options are allowlisted and sent as Luacid query parameters and WebSocket request options.</div>
        <div class="decompiler-modal-footer"><span class="decompiler-modal-note" data-provider-save-status>Saved automatically</span><button class="modal-btn modal-btn--cancel" type="button" data-action="close-provider-modal">Close</button></div>
    `;
}
...
async function saveDecompilerProviderModal(options = {}) {
    ...
    if (id === 'oracle') {
        ...
        try {
            ...
            for (const [optionKey, value] of Object.entries(parsed)) {
                const validValue = typeof value === 'boolean'
                    || (typeof value === 'string' && value.length <= 80)
                    || (typeof value === 'number' && Number.isFinite(value) && Math.abs(value) <= 1000000);
                if (!(optionKey in LUACID_DEFAULT_OPTIONS) || !validValue) throw new Error();
            }
            const transport = ['http', 'websocket'].includes($('decompilerModalLuacidTransport')?.value)
                ? $('decompilerModalLuacidTransport').value
                : 'auto';
            provider.options = { ...parsed, transport, transportExplicit: transport !== 'auto' };
        } catch {
            setDecompilerProviderSaveStatus('Not saved: use only supported primitive Luacid options.', true);
            if (!options.silent) showToast('Luacid options are invalid', 'error');
            return false;
        }

...
const EMBEDDING_RESPONSE_LIMIT_BYTES = 32 * 1024 * 1024;
...
function assertVectors(value: unknown, provider: string): number[][] {
  ...
  return value.map((embedding, index) => {
    ...
    if (embedding.length !== dimensions) {
      throw new EmbeddingProviderError(`${provider} returned inconsistent embedding dimensions.`);
    }
...
async function readJsonResponse<T>(response: Response, provider: string): Promise<T> {
  const raw = await readBoundedResponseText(response, EMBEDDING_RESPONSE_LIMIT_BYTES);
  let parsed: unknown = {};
...
async function fetchEmbeddingJson<T>(
  url: string,
  init: RequestInit,
  provider: string
): Promise<T> {
  ...
  try {
    ...
    return await readJsonResponse<T>(response, provider);
  } catch (error) {
    if (error instanceof EmbeddingProviderError) throw error;
    if (error instanceof Error && error.name === "AbortError") {
      throw new EmbeddingProviderError(`${provider} embedding request timed out after 60 seconds.`);
    }
    throw new EmbeddingProviderError(
      `${provider} embedding request failed: ${error instanceof Error ? error.message : String(error)}`
    );
  } finally {
...
async function embedWithOpenAI(settings: SemanticSettings, inputs: string[]): Promise<number[][]> {
  ...
  const parsed = await fetchEmbeddingJson<OpenAIEmbeddingResponse>(
    `${baseUrl}/embeddings`,
    {
      method: "POST",
      headers: {
...
async function embedWithOllama(settings: SemanticSettings, inputs: string[]): Promise<number[][]> {
  ...
  const parsed = await fetchEmbeddingJson<OllamaEmbeddingResponse>(
    `${baseUrl}/api/embed`,
    {
      method: "POST",

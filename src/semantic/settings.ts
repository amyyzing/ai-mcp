...
export function semanticRemoteUploadDisclosure(
  settings: SemanticSettings
): string | null {
  if (settings.provider !== "openai") return null;

  return (
    "The configured OpenAI-compatible embedding endpoint will receive source-derived " +
    `script text${settings.openaiBaseUrl ? ` at ${settings.openaiBaseUrl}` : ""} and, for searches, the query. ` +
    "This can disclose game code to that provider and may incur API usage charges. " +
    "Set confirmRemoteEmbeddingUpload=true to confirm this upload and potential cost."
  );
}

export function validateSemanticUploadConfirmation(
  settings: SemanticSettings,
  confirmed: unknown
): string | null {
  const disclosure = semanticRemoteUploadDisclosure(settings);
  return disclosure && confirmed !== true ? disclosure : null;
}

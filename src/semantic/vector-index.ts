...
const MAX_SOURCE_TEMPLATE_CACHE_ENTRIES = 512;
const MAX_SOURCE_TEMPLATE_CACHE_CHARS = 16 * 1024 * 1024;
export const MAX_SEMANTIC_CHUNKS = 5_000;
export const MAX_SEMANTIC_PROVIDER_INPUT_CHARS = 4 * 1024 * 1024;
const MAX_SEMANTIC_CHUNK_MEMORY_CHARS = 16 * 1024 * 1024;
const MAX_VECTOR_SESSIONS = 8;
const MAX_VECTOR_VALUES_IN_MEMORY = 8_000_000;
const MAX_SEMANTIC_EMBEDDING_WORK_MS = 5 * 60_000;
...
let sourceChunkTemplateChars = 0;

function templateChars(chunks: SourceChunkTemplate[]): number {
  return chunks.reduce(
    (total, chunk) =>
      total +
      chunk.body.length +
      chunk.semanticText.length +
      chunk.lexicalText.length +
      chunk.label.length +
      chunk.summary.length +
      chunk.features.reduce((featureTotal, feature) => featureTotal + feature.length, 0),
    0
  );
}

function cacheSourceChunkTemplates(
  cacheKey: string,
  chunks: SourceChunkTemplate[]
): void {
  const chars = templateChars(chunks);
  if (chars > MAX_SOURCE_TEMPLATE_CACHE_CHARS) return;

  sourceChunkTemplatesByHash.set(cacheKey, chunks);
  sourceChunkTemplateChars += chars;
  while (
    sourceChunkTemplatesByHash.size > MAX_SOURCE_TEMPLATE_CACHE_ENTRIES ||
    sourceChunkTemplateChars > MAX_SOURCE_TEMPLATE_CACHE_CHARS
  ) {
    const oldestKey = sourceChunkTemplatesByHash.keys().next().value as string | undefined;
    if (oldestKey === undefined) break;
    const oldest = sourceChunkTemplatesByHash.get(oldestKey);
    sourceChunkTemplatesByHash.delete(oldestKey);
    if (oldest) sourceChunkTemplateChars -= templateChars(oldest);
  }
}
...
function chunkTemplatesForSource(script: StoredScriptSource): SourceChunkTemplate[] {
  ...
  if (cached) {
    ...
    sourceChunkTemplatesByHash.delete(cacheKey);
    sourceChunkTemplatesByHash.set(cacheKey, cached);
    return cached;
  }

  const chunks = buildSemanticChunkTemplates(script);
  cacheSourceChunkTemplates(cacheKey, chunks);
  return chunks;
}
...
function buildChunks(scripts: StoredScriptSource[]): ScriptChunk[] {
  ...
  let providerInputChars = 0;
  let retainedChars = 0;

  for (const script of scripts) {
    for (const chunk of chunkScript(script)) {
      providerInputChars += chunk.semanticText.length;
      retainedChars += chunk.body.length + chunk.semanticText.length + chunk.lexicalText.length;
      if (
        chunks.length >= MAX_SEMANTIC_CHUNKS ||
        providerInputChars > MAX_SEMANTIC_PROVIDER_INPUT_CHARS ||
        retainedChars > MAX_SEMANTIC_CHUNK_MEMORY_CHARS
      ) {
        throw new Error(
          `Semantic index capacity exceeded (${chunks.length + 1} chunks, ` +
          `${providerInputChars} provider-input characters). Narrow the script index before semantic search; ` +
          `limits are ${MAX_SEMANTIC_CHUNKS} chunks and ${MAX_SEMANTIC_PROVIDER_INPUT_CHARS} provider-input characters.`
        );
      }
      chunks.push(chunk);
    }
...
function getOrCreateSession(key: string): SemanticVectorSession {
  const existing = vectorSessionsByKey.get(key);
  if (existing) {
    vectorSessionsByKey.delete(key);
    vectorSessionsByKey.set(key, existing);
    return existing;
  }

  while (vectorSessionsByKey.size >= MAX_VECTOR_SESSIONS) {
    const oldestKey = vectorSessionsByKey.keys().next().value as string | undefined;
    if (oldestKey === undefined) break;
    vectorSessionsByKey.delete(oldestKey);
  }
  const session: SemanticVectorSession = { vectors: new Map(), vectorValues: 0 };
  vectorSessionsByKey.set(key, session);
  return session;
}

function totalVectorValues(): number {
  let total = 0;
  for (const session of vectorSessionsByKey.values()) total += session.vectorValues;
  return total;
}

function setSessionVector(
  session: SemanticVectorSession,
  embeddingId: string,
  embedding: number[]
): void {
  const previous = session.vectors.get(embeddingId);
  const addedValues = embedding.length - (previous?.length ?? 0);
  while (addedValues > 0 && totalVectorValues() + addedValues > MAX_VECTOR_VALUES_IN_MEMORY) {
    const oldest = vectorSessionsByKey.entries().next().value as
      | [string, SemanticVectorSession]
      | undefined;
    if (!oldest || oldest[1] === session) break;
    vectorSessionsByKey.delete(oldest[0]);
  }
  if (addedValues > 0 && totalVectorValues() + addedValues > MAX_VECTOR_VALUES_IN_MEMORY) {
    throw new Error(
      `Semantic vector memory capacity exceeded (${MAX_VECTOR_VALUES_IN_MEMORY} numeric values). ` +
      "Use a smaller script index or a lower-dimensional embedding model."
    );
  }
  if (previous) session.vectorValues -= previous.length;
  session.vectors.delete(embeddingId);
  session.vectors.set(embeddingId, embedding);
  session.vectorValues += embedding.length;
}
...
function pruneStaleVectors(session: SemanticVectorSession, chunks: ScriptChunk[]): void {
  ...
  for (const embeddingId of session.vectors.keys()) {
    if (!currentEmbeddingIds.has(embeddingId)) {
      const embedding = session.vectors.get(embeddingId);
      session.vectors.delete(embeddingId);
      session.vectorValues -= embedding?.length ?? 0;
    }
  }
}
...
async function embedMissingChunks(
  ...
  if (settings.saveEmbeddingsToDisk) {
    for (const chunk of uniqueChunks) {
      ...
      setSessionVector(session, chunk.embeddingId, embedding);
      loadedFromDisk += 1;
    }
  }
  ...
  for (let i = 0; i < toEmbed.length; i += batchSize) {
    if (Date.now() - workStartedAt > MAX_SEMANTIC_EMBEDDING_WORK_MS) {
      throw new Error("Semantic embedding work exceeded the five-minute job budget.");
    }
...
export async function semanticSearchScripts(
  ...
  if (settings.saveEmbeddingsToDisk) {
    ...
    for (const chunk of uniqueChunks) {
      ...
      if (embedding) setSessionVector(session, chunk.embeddingId, embedding);
    }
  }
...
export async function semanticIndexCodebase(
  ...
  return {
    ...
    embeddedChunks,
    sourceIndexComplete,
    isPartialIndex: embeddedChunks < chunks.length || !sourceIndexComplete,
  };
}
...
export function clearAllSemanticIndexes(): void {
  ...
  sourceChunkTemplatesByHash.clear();
  sourceChunkTemplateChars = 0;
}

export function getSemanticTemplateCacheStats(): {
  entries: number;
  characters: number;
} {
  return {
    entries: sourceChunkTemplatesByHash.size,
    characters: sourceChunkTemplateChars,
  };
}

export function getSemanticVectorCacheStats(): {
  sessions: number;
  vectors: number;
  vectorValues: number;
} {
  let vectors = 0;
  for (const session of vectorSessionsByKey.values()) vectors += session.vectors.size;
  return {
    sessions: vectorSessionsByKey.size,
    vectors,
    vectorValues: totalVectorValues(),
  };
}

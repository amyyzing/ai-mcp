...
From the MCP, call `devirtualize-luraph` with `operation=run`, page the returned result with `operation=read`, and remove the in-memory result with `operation=release`. Cached results expire after 10 minutes.
